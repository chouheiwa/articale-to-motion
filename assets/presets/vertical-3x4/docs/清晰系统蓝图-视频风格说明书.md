...
![命题型示例](../assets/style-guide/examples/proposition.png)

### 9.2 对照型

用于旧方法与新方法、原料与结果、条件与结论。左右或上下形成明确对照，最终结论单独占一层，不塞进比较卡。

![对照型示例](../assets/style-guide/examples/comparison.png)

### 9.3 流程型

用于步骤、层级、因果和状态变化。使用纵向轨道、节点和状态胶囊，不做普通项目符号列表。

![流程型示例](../assets/style-guide/examples/process.png)

### 9.4 能力板型

用于系统、能力、工程资产和验证结果。深蓝板成为视觉锚点，周围保留浅色说明和结构关系。

![能力板型示例](../assets/style-guide/examples/capability_deck.png)
...
`main` 只保留可跨项目复用的风格规范、骨架示例和自动校验工具，不包含任何单片的稿件、镜头、音频或制作证据。
